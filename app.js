...
class CVDownloadManager {
  ...
  downloadCV() {
    ...
    link.download = "Ahmed_Elsayed_Abdel-Al_CV.txt";
...
// Picture Container Animation
class PictureContainerManager {
  constructor() {
    this.init();
  }
  ...
  init() {
    ...
    if (pictureContainer) {
      this.setupInteractions(pictureContainer);
    }
  }

  setupInteractions(container) {
    const placeholder = container.querySelector(".picture-placeholder");

    if (placeholder) {
      placeholder.addEventListener("mouseenter", () => {
        placeholder.style.transform = "scale(1.05) rotate(5deg)";
      });
